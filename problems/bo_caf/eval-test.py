...
class CustomAF(AnalyticAcquisitionFunction):
    def __init__(self, model, cost_model, train_x, train_Y, best_x, best_Y,
    ...
    def forward(self, test_x: Tensor) -> Tensor:
        ...
        utility_value = utility(self.train_x, self.train_Y,
...
class Evaluation():
    ...
    def EGO(self, cost_total, real_problem, random_seed):
        with warnings.catch_warnings():
            ...
            while cost_used < cost_total:
                ...
                # choose af
                # EvolCAF
                ei = CustomAF(model, cost_model, X_ei, train_Y, best_x, best_Y,
...
def EvalEGO(interface_eval):
    N_TRIALS = 10
    # num_cores = multiprocessing.cpu_count()
    time_limit = 100000  # maximum 60 seconds for BO loop when training, can be set much larger when testing other AFs
    gap_all = Parallel(n_jobs=30, timeout=time_limit)(
        delayed(interface_eval.EGO)(cost_total=30, real_problem=instance, random_seed=trial)
        for trial in range(1, N_TRIALS + 1)
        for instance in interface_eval.instance_data
    )
    gap_all = np.array(gap_all).reshape(N_TRIALS, len(interface_eval.instance_data))
    print(gap_all)
...
if __name__ == "__main__":
    ...
    data = os.path.join(basepath, f'instance/botorch_problem_train.pkl')
    interface_eval = Evaluation(data)
    # print(source_code)
    fitness = EvalEGO(interface_eval)
